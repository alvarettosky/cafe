...
export function NewSaleModal({
  ...
  useEffect(() => {
    if (!isRepeatSale) {
      const suggestedPrice = unit === 'libra' ? 10.0 : 5.0;
  ...
  const fetchCustomerRecurrence = useCallback(async () => {
    ...
    try {
      ...
      if (customer?.typical_recurrence_days) {
        ...
        const { data, error } = await supabase.rpc('calculate_customer_recurrence', {
        ...
        <div className="grid gap-3 py-3 overflow-y-auto flex-1 pr-2">
          ...
          <div className="flex flex-col space-y-2">
            ...
            <select
              ...
              value={productId}
            >
              <option value="" className="text-black dark:text-white bg-white dark:bg-zinc-900">
                Seleccionar Café...
              </option>
              {products.map(p => (
                <option
                  key={p.product_id}
                  value={p.product_id}
                  className="text-black dark:text-white bg-white dark:bg-zinc-900"
                >
                  {p.product_name}
                </option>
              ))}
          ...
          <div className="flex flex-col space-y-2">
            ...
            {customerPriceInfo &&
              customerPriceInfo.discount_applied &&
              customerPriceInfo.discount_applied > 0 && (
                <div className="flex items-center gap-2 p-2 bg-green-50 dark:bg-green-900/20 rounded-md border border-green-200 dark:border-green-800">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-green-600 dark:text-green-500">
                      {customerPriceInfo.price_list_name ||
                        customerPriceInfo.customer_type ||
                        'Lista especial'}
                    </p>
                  </div>
          ...
          {error && (
            <div className="p-2.5 text-sm text-red-500 bg-red-500/10 rounded-md">{error}</div>
          )}
        </div>
