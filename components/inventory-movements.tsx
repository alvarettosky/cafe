...
export function InventoryMovements({
  ...
  const [movementType, setMovementType] = useState<'restock' | 'loss' | 'adjustment' | 'return'>(
    'restock'
  );
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gray-50 rounded-lg p-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">{formatGrams(product.total_grams_available)}</span>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <select
                  value={movementType}
                  onChange={e =>
                    setMovementType(e.target.value as 'restock' | 'loss' | 'adjustment' | 'return')
                  }
              ...
              <div className="space-y-1">
                <Label className="text-sm">
                  ...
                  {movementType === 'adjustment' && (
                    <span className="text-xs text-gray-500 ml-1">(negativo para restar)</span>
                  )}
                </Label>
                <Input
                  type="number"
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">
                Los movimientos se registrarán automáticamente con cada venta
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${movementTypeColors[movement.movement_type]}`}
                          >
                            {movementTypeLabels[movement.movement_type]}
                          </span>
                          <span
                            className={`font-semibold ${movement.quantity_grams >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {movement.quantity_grams >= 0 ? '+' : ''}
                            {formatGrams(movement.quantity_grams)}
                          </span>
                        </div>
                        {movement.reason && (
                        ...
                        <div className="flex items-center gap-4 mt-1 text-xs text-gray-400">
                          <span>
                            {formatGrams(movement.stock_before)} →{' '}
                            {formatGrams(movement.stock_after)}
                          </span>
                          {movement.batch_number && <span>Lote: {movement.batch_number}</span>}
        ...
        <div className="border-t pt-3 mt-3">
          <div className="flex items-center gap-1 text-xs text-gray-500">
            ...
            <span>
              Los movimientos de venta se registran automáticamente al procesar cada venta.
            </span>
          </div>
        </div>
      </DialogContent>
