          ...
          return (
            <optgroup key={productId} label={productName}>
              {productVariants.map(variant => {
                ...
                const grindLabel =
                  grindTypeShortLabels[variant.grind_type as keyof typeof grindTypeShortLabels] ||
                  variant.grind_type;
                const unitsAvailable = stockInUnits(variant.stock_grams, variant.weight_grams);
                const stockText = showStock ? ` [${unitsAvailable} disponibles]` : '';
                ...
                return (
                  ...
                  >
                    {presLabel} ({grindLabel}) - ${variant.base_price.toFixed(2)}
                    {stockText}
                  </option>
                );
              })}
      ...
      {selectedVariant && (
        <div className="flex items-center justify-between p-2 bg-muted/30 rounded-md text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            {showStock && (
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStockLevelColor(selectedVariant.stock_grams, selectedVariant.weight_grams * 5)}`}
              >
