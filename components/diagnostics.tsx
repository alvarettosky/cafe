...
export function Diagnostics() {
  ...
  return (
    <div className="fixed bottom-4 right-4 max-w-sm p-4 rounded-lg shadow-2xl bg-zinc-900 border border-red-500 text-white z-50">
      <h3 className="font-bold text-red-500 flex items-center gap-2">⚠️ Diagnóstico del Sistema</h3>
      <p className="text-sm mt-1">{message}</p>
      {details && (
        <pre className="mt-2 p-2 bg-black rounded text-xs overflow-auto max-h-32">{details}</pre>
      )}
    </div>
  );
}
