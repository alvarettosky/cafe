...
describe('ProductVariantSelector', () => {
  ...
  it('should show loading state initially', () => {
    render(<ProductVariantSelector onChange={mockOnChange} />);
  ...
  it('should show stock availability in options when showStock is true', async () => {
    render(<ProductVariantSelector onChange={mockOnChange} showStock={true} />);
    ...
    await waitFor(() => {
      ...
      const optionWithStock = options.find(opt => opt.textContent?.includes('disponibles'));
      expect(optionWithStock).toBeDefined();
    });
  });

  it('should not show stock when showStock is false', async () => {
    render(<ProductVariantSelector onChange={mockOnChange} showStock={false} />);
  ...
  it('should call onChange with null when deselected', async () => {
    const user = userEvent.setup();
    render(<ProductVariantSelector value="var-1" onChange={mockOnChange} />);
  ...
  it('should show stock units in info panel', async () => {
    render(<ProductVariantSelector value="var-1" onChange={mockOnChange} showStock={true} />);
  ...
  it('should disable options without stock when showStock is true', async () => {
    ...
    await waitFor(() => {
      ...
      const outOfStockOption = options.find(opt => opt.getAttribute('value') === 'var-5');
      expect(outOfStockOption).toBeDisabled();
    });
  });

  it('should be disabled when disabled prop is true', async () => {
    render(<ProductVariantSelector onChange={mockOnChange} disabled={true} />);
  ...
  it('should apply custom className', async () => {
    const { container } = render(
      <ProductVariantSelector onChange={mockOnChange} className="custom-class" />
    );
  ...
  it('should display price in variant options', async () => {
    ...
    await waitFor(() => {
      ...
      const optionWithPrice = options.find(
        opt => opt.textContent?.includes('$10.00') || opt.textContent?.includes('$15.00')
      );
...
describe('ProductVariantSelectCompact', () => {
  ...
  it('should be disabled when disabled prop is true', async () => {
    render(<ProductVariantSelectCompact onChange={mockOnChange} disabled={true} />);
  ...
  it('should apply custom className', async () => {
    render(<ProductVariantSelectCompact onChange={mockOnChange} className="my-custom-class" />);
