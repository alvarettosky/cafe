...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'top_products', (
            SELECT COALESCE(jsonb_agg(product_data ORDER BY revenue DESC), '[]'::jsonb)
            FROM (
                SELECT jsonb_build_object(
...
-- Test the functions
SELECT 'Testing get_advanced_metrics' as test_name, get_advanced_metrics() as result;
SELECT 'Testing get_sales_time_series' as test_name, * FROM get_sales_time_series() LIMIT 5;
SELECT 'Testing get_product_performance' as test_name, * FROM get_product_performance() LIMIT 5;
