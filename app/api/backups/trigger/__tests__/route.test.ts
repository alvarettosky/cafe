...
describe('Backup Trigger API Route', () => {
  ...
  describe('Authorization', () => {
    ...
    it('should return 500 (not 403) when the profile fetch itself fails', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error fetching profile for backups trigger:',
        profileError
      );
  ...
  describe('GitHub Token Configuration', () => {
    ...
    it('should return 400 when GitHub token is not configured', async () => {
      ...
      expect(data.message).toBe(
        'Configure GITHUB_TOKEN en las variables de entorno para habilitar backups manuales'
      );
    });
  ...
  describe('Successful Backup Trigger', () => {
    ...
    it('should successfully trigger backup via GitHub Actions', async () => {
      ...
      server.use(
        http.post(
          'https://api.github.com/repos/alvarettosky/cafe/actions/workflows/daily-backup.yml/dispatches',
          () => {
    ...
    it('should use custom repository when GITHUB_REPOSITORY is set', async () => {
      ...
      server.use(
        http.post(
          'https://api.github.com/repos/myorg/myrepo/actions/workflows/daily-backup.yml/dispatches',
          () => {
            return new HttpResponse(null, { status: 204 });
          }
        )
