...
describe('PreciosPage', () => {
  ...
  it('should show restricted access message for non-admin users', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Solo administradores pueden acceder a esta sección/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('should display information box about pricing system', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Los precios se aplican automáticamente según el tipo de cliente/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('should display customer types in info box', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Retail, Mayorista Pequeño, Mayorista Grande, Cafetería, Personalizado/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('should show subtitle about managing differentiated prices', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Gestiona precios diferenciados por tipo de cliente/i)
      ).toBeInTheDocument();
    });
  });
