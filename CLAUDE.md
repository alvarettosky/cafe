...
| [`scripts/restore-drill.sh`](scripts/restore-drill.sh)               | `./scripts/restore-drill.sh` | Que el backup **no se pueda restaurar**. Reconstruye la base en un Postgres efímero desde el esquema del propio ZIP, le carga el último backup real y **compara con producción objetos, funciones y —desde el 2026-08-09— columnas**. Cazó que el backup respaldaba 20 de 21 tablas, que el esquema no era reconstruible, y dos columnas que existían solo en producción |
