...
| A22 | **La suite unitaria no es hermética: lee el entorno**             | Con `SUPABASE_SERVICE_ROLE_KEY` y `NEXT_PUBLIC_SUPABASE_URL` exportadas, **30 tests de 6 archivos fallan**; en una terminal limpia pasan los 889. Medido el 2026-08-09 corriendo `npm test` dos veces seguidas, lo único distinto el entorno. Un test que cambia de veredicto según quién lo lance mide la máquina, no el código — y el día que falle de verdad, nadie lo creerá. Fixture `autouse` que fije los valores DECLARADOS en vez de heredarlos, y un test que compruebe el aislamiento en sí                        |
| A23 | **Catálogo de productos con nombres truncados y precios de demo** | Los 5 `products` del POS se llaman «Café», «Café en», «Café Tostado» —nombres partidos, probablemente por un `split` en la migración de datos— y sus variantes cuestan `10.00` y `5.50`, que no son pesos: la libra real se vendió a **$45.000** en 2026. Además el portal público lista **4 productos del modelo viejo** (`inventory`), distintos de los 5 del POS: dos catálogos que no coinciden. Bloquea cualquier carga del histórico, porque ataría cada venta a un producto equivocado                                 |
...
| C6  | **Cargar el histórico real al CRM**             | La base de producción tiene **datos de prueba**, no el negocio: 2 clientes («Profe Vanesa» **no existe** en el CSV real; «El mono» no casa con «Mono (FruVer de la 50)»), 1 venta, precios en dólares de demo. El histórico verdadero —**53 clientes · 133 ventas · 159 libras · $6.087.500 vendidos frente a $4.895.500 cobrados**, del 2024-09-26 al 2026-06-03— vive en `ventas-y-pagos-cafe-*.csv`, en el repo **privado**. Mientras no se cargue, la segmentación RFM, la recurrencia y la cartera no tienen con qué trabajar. Requiere **A23 primero** (catálogo sano) y decidir qué hacer con los 2 clientes de prueba |
