...
| A21 | **El portal listaría productos DUPLICADOS**                 | Destapado al arreglar A15: con la RPC ya funcionando devuelve **10 entradas para 4 productos** (`Café Molido Medio` ×3, `Café Tostado (Grano)` ×3, los otros dos ×2). `inventory` conserva una fila por lote y **las 10 están migradas al modelo de variantes** (`migrated_to_product_id` no nulo en todas). No se puede deduplicar dentro de la RPC sin decidir qué stock cuenta: `create_customer_order` valida contra la fila concreta (`inventory.product_id`), así que agrupar por nombre haría que un producto con 3 lotes de 2500 g se valide contra 2500 y no contra 7500. Las dos salidas reales: (a) el portal pasa al modelo nuevo (`products`/`product_variants`, como `get_variants_for_sale`), o (b) se consolida `inventory`. Es decisión de arquitectura, no un fix |
